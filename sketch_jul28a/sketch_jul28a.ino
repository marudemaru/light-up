// FastLEDライブラリをインクルードします。このライブラリがLEDの制御を簡単にしてくれます。
#include <FastLED.h>

// --- 基本設定 ---
#define DATA_PIN 6      // LEDテープのデータ信号を送るArduinoのピン番号
#define NUM_LEDS 300    // 使用するLEDの総数
#define BRIGHTNESS 64   // LED全体の明るさを設定 (0から255の範囲で指定)

// --- 流れる光の設定 ---
const CRGB COMMON_COL[] = {
    CRGB::Snow, CRGB::DeepSkyBlue, CRGB::Snow,
    CRGB::HotPink, CRGB::Snow, CRGB::Gold
};
const int COMMON_COL_SIZE = sizeof(COMMON_COL) / sizeof(COMMON_COL[0]);

// 各光の設定
#define CHANGETIME1 1000
#define FLOW_SPEED1 1
#define CHANGETIME2 1000
#define FLOW_SPEED2 40
#define CHANGETIME3 1000
#define FLOW_SPEED3 80
#define CHANGETIME4 1000
#define FLOW_SPEED4 120

// --- 背景（キラキラ）の設定 ---
#define BG_CHANGETIME 10000
const CRGB BG_COL[] = {
    CRGB::Red, CRGB::LightGreen, CRGB::LightSkyBlue,
    CRGB::Plum, CRGB::Aquamarine, CRGB::Olive
};
...
void loop() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (bg_br_delta[i] > 0 && bg_br[i] >= BRIGHTNESS - 1) {
      bg_br_delta[i] = -1;
    } else if (bg_br_delta[i] < 0 && bg_br[i] <= 30) {
      bg_br_delta[i] = 1;
    }
